# Patched version of the upstream home-manager mpv module.
# Changes from upstream:
#   - package option is nullable (lib.mkPackageOption with nullable = true)
#   - home.packages and finalPackage are guarded against null package
#   - assertion allows null package
# This allows setting programs.mpv.package = null on non-NixOS hosts
# to skip package installation while keeping config file management.
{
  config,
  lib,
  pkgs,
...
let
  ...
  mpvPackage =
    ...
    else if cfg.scripts == [ ] then cfg.package
    ...
    lib.mkMerge [
      {
        assertions = [
          {
            assertion = (cfg.package == null) || (cfg.scripts == [ ]) || (cfg.package == pkgs.mpv);
            message = ''The programs.mpv "package" option is mutually exclusive with "scripts" option.'';
          }
        ];
      }
      {
        home.packages = mkIf (mpvPackage != null) [ mpvPackage ];
